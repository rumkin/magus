...
/**
 * Execute function in actions sequence (in nodejs context).
...
/**
 * Define internal variable action. This value will be passed to each tab page. All values accessible from window.MAGUS.
...
Browser.prototype.set = function(name, value) {
    ...
    this.exec(function(result, done){
        ...
        if (! window.MAGUS) window.MAGUS = {};
        window.MAGUS[name] = value;
    });
    return this;
};
...
Browser.prototype.click = function(query) {
    this.eval(query, function(query){
        ...
        var target = query ? document.querySelector(query) : MAGUS.this;
...
Browser.prototype.submit = function(query) {
    this.eval(query, function(query){
        var form = query ? document.querySelector(query) : MAGUS.this;
...
Browser.prototype.attr = function(name, value) {
    if (arguments.length < 2) {
        this.getAttribute(null, name);
    } else {
...
Browser.prototype.setAttribute = function(query, name, value) {
    this.eval(query, name, value, function(query, name, value){
        var target = query ? document.querySelector(query) : MAGUS.this;
...
Browser.prototype.getAttribute = function(query, name) {
    this.eval(query, name, function(query, name){
        var target = query ? document.querySelector(query) : MAGUS.this;
...
Browser.prototype.download = function(destination) {
    this.exec(function(url, done){
        if (! url) return done(new Error('Invalid url'));

        this.currentTab().page.getCookies(function (cookies) {
            var cookie = cookies.map(function(cookie){
